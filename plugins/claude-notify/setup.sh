...
if ! [[ "$PORT" =~ ^[0-9]+$ ]] || [ "$PORT" -lt 1 ] || [ "$PORT" -gt 65535 ]; then
    echo "  ! Invalid port '$PORT'. Falling back to 17839."
    PORT="17839"
fi
echo "  → Port: $PORT"
...
for mod in "${MOD_ARRAY[@]}"; do
    ...
    [ -z "$mod" ] && continue
    mod_escaped=$(lua_escape "$mod")
    LUA_MODS="${LUA_MODS}\"${mod_escaped}\", "
done
LUA_MODS="${LUA_MODS%, }"
if [ -z "$LUA_MODS" ]; then
    LUA_MODS="\"ctrl\", \"shift\""
...
TERMINAL_ESCAPED=$(lua_escape "$TERMINAL")
HS_PATH_ESCAPED=$(lua_escape "$HS_PATH")
TMUX_PATH_ESCAPED=$(lua_escape "$TMUX_PATH")
HOTKEY_KEY_ESCAPED=$(lua_escape "$HOTKEY_KEY")

cat > "$CONFIG_LUA" << LUAEOF
-- Claude Notify Config (generated by setup)
return {
    terminal = "${TERMINAL_ESCAPED}",
    hs_path = "${HS_PATH_ESCAPED}",
    tmux_path = "${TMUX_PATH_ESCAPED}",
    use_tmux = ${USE_TMUX},
    port = ${PORT},
    hotkey_mods = { ${LUA_MODS} },
    hotkey_key = "${HOTKEY_KEY_ESCAPED}",
    lang = "${LANG_CODE}",
    theme = "${THEME}",
    panel = {
        width = 320,
